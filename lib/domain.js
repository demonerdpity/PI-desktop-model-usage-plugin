...
function safeProvenanceEntry(input) {
  if (!input || typeof input !== "object" || Array.isArray(input)) return null;
  const output = {};
  if (typeof input.available === "boolean") output.available = input.available;
  if (typeof input.estimated === "boolean") output.estimated = input.estimated;
  const sourceType = scalarText(input.sourceType, 80);
  if (sourceType) output.sourceType = sourceType;
  const collectedAt = normalizeTimestamp(input.collectedAt);
  if (collectedAt !== null) output.collectedAt = collectedAt;
  const coverage = finiteNonNegative(input.coverage);
  if (coverage !== undefined && coverage !== null && coverage <= 1) output.coverage = coverage;
  const reason = scalarText(input.reason, 320);
  const note = scalarText(input.note, 320);
  if (reason) output.reason = reason;
  if (note) output.note = note;
  if (typeof input.endpoint === "string") {
    try {
      const url = new URL(input.endpoint);
      if (url.protocol === "https:" && !url.username && !url.password) output.endpoint = `${url.origin}${url.pathname}`;
    } catch {
      // Invalid or non-public endpoint metadata is omitted.
    }
  }
  return output;
}

function channelProvenance(input) {
  if (!input || typeof input !== "object" || Array.isArray(input)) return {};
  const output = {};
  for (const key of ["usage", "cost", "quota"]) {
    const entry = safeProvenanceEntry(input[key]);
...
function quotaWindow(input) {
  ...
  const resetAfterSeconds = finiteNonNegative(input.resetAfterSeconds);
  if ([used, limit, remaining, usedPercent, remainingPercent, resetAfterSeconds].includes(null)) return null;
  if (
    used === undefined && limit === undefined && remaining === undefined &&
    usedPercent === undefined && remainingPercent === undefined &&
    resetAt === null && resetAfterSeconds === undefined
  ) return null;
  return {
    label: scalarText(input.label, 80, "Quota"),
    ...(used !== undefined ? { used } : {}),
...
function creditUsage(input) {
  ...
  const resetAt = input.resetAt == null ? null : normalizeTimestamp(input.resetAt);
  const resetAfterSeconds = finiteNonNegative(input.resetAfterSeconds);
  const balance = scalarText(input.balance, 80);
  const unlimited = input.unlimited === true;
  if ([total, used, remaining, remainingPercent, resetAfterSeconds].includes(null)) return null;
  if (
    total === undefined && used === undefined && remaining === undefined &&
    remainingPercent === undefined && resetAt === null &&
    resetAfterSeconds === undefined && !balance && !unlimited
  ) return null;
  return {
...
/**
 * Manual "rate limit reset" passes the provider grants an account. Metadata
 * only: the dashboard never spends one.
 */
function resetCredits(input) {
  if (!input || typeof input !== "object" || Array.isArray(input)) return null;
  const available = finiteNonNegative(input.availableCount, { integer: true });
  const applicable = finiteNonNegative(input.applicableCount, { integer: true });
  if (available === undefined && applicable === undefined) return null;
...
function channelSnapshot(input = {}) {
  const quotaWindows = Array.isArray(input.quotaWindows)
    ? input.quotaWindows.map(quotaWindow).filter(Boolean).slice(0, 8)
    : [];
  const legacyQuota = quotaWindow(input.quota);
  if (legacyQuota && !quotaWindows.length) quotaWindows.push(legacyQuota);
  return {
    ...
    accountLabel: scalarText(input.accountLabel, MAX_PROVIDER_LENGTH),
    plan: scalarText(input.plan, 80),
    ...
    quota: legacyQuota,
    quotaWindows,
    creditUsage: creditUsage(input.creditUsage),
    resetCredits: resetCredits(input.resetCredits),
    // Marks a card whose quota comes from the provider's own subscription API
    // rather than from local token history. The renderer keys its quota section
    // off this flag so an unavailable subscription still explains itself.
    subscriptionQuota: input.subscriptionQuota === true,
    quotaAvailable: quotaWindows.length > 0,
    capabilities: capabilities({ ...input.capabilities, quota: quotaWindows.length > 0, reset: quotaWindows.some((window) => window.resetAt != null || window.resetAfterSeconds != null) }),
    provenance: channelProvenance(input.provenance),
  };
}

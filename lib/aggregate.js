(function (root, factory) {
  ...
  function mergeQuotaChannels(providerList, quotaInput) {
    ...
    for (const raw of Array.isArray(quotaInput) ? quotaInput : []) {
      ...
      channels.set(channel.id, {
        ...(existing || channel),
        id: channel.id,
        label: channel.label || existing?.label || channel.id,
  ...
  function aggregate(factsInput, options = {}) {
    ...
    const providers = mergeQuotaChannels(breakdown.providers, options.quotaChannels);
    ...
    return {
      ...
      provenance: {
        ...
        quota: providers.some((provider) => provider.quotaAvailable)
          ...
          : { available: false, reason: "The current host does not provide quota, balance, or reset data to plugins." },
      },
